...
def test_render_template_success() -> None:
    ...
    render.load_template_file(template_file).apply_context(context)
...
def test_render_template_not_strict_undefined_success() -> None:
    ...
    render.load_template_file(template_file).apply_context({})
    result = render.render_content
    assert result == "Hello !"
...
def test_render_template_with_not_binary_error() -> None:
    ...
    context = {"name": "World"}
    render = GhostwriterRender()
    render.load_template_file(template_file).apply_context(context)  # type: ignore
    result = render.render_content
    assert result is None
